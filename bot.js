...
//addUserMsg
	client.on('messageReactionAdd',(reaction,user)=>{
		...
		if(rolename==="🔞"&&member.roles.has("676117026271395846")==0){
				var member = reaction.message.guild.members.find(member=>member.id===user.id) ;
				member.addRole("676117026271395846").catch(err=>console.error) ;
				reaction.remove(member) ;
		}
		if(rolename==="0️⃣"&&member.roles.has("672471249653465121")==0){
				var member = reaction.message.guild.members.find(member=>member.id===user.id) ;
				member.addRole("672471249653465121").catch(err=>console.error) ;
				reaction.remove(member) ;
		}
		if(rolename==="1️⃣"&&member.roles.has("661229449450356746")==0){
				var member = reaction.message.guild.members.find(member=>member.id===user.id) ;
				member.addRole("661229449450356746").catch(err=>console.error) ;
				reaction.remove(member) ;
		}
		if(rolename==="2️⃣"&&member.roles.has("661204081830002694")==0){
				var member = reaction.message.guild.members.find(member=>member.id===user.id) ;
